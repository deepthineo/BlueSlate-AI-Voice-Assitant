...
import { VAPI_PHONE, VAPI_PHONE_HREF } from '../lib/vapi';

// ──────────────────────────────────────────────────────────────
// "Call AI Receptionist" — the Vapi inbound number with click-to-call.
// Reusable: drop <CallReceptionist /> on the landing, demo, receptionist,
// or any page where calling is promoted. Same number, same assistant.
// ──────────────────────────────────────────────────────────────
...
export default function CallReceptionist({ variant = 'block', className = '' }: Props) {
  if (variant === 'inline') {
    return (
      <a href={VAPI_PHONE_HREF}
        ...
        <span className="tabular-nums">{VAPI_PHONE}</span>
      </a>
    );
  }
  ...
  return (
    <a href={VAPI_PHONE_HREF}
      ...
      <span className="text-white font-bold text-xl tabular-nums">{VAPI_PHONE}</span>
    </a>
  );
}
